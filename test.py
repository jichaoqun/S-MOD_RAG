...
#     def create_table(self, table_name: str, data: List[Dict], vector_column: str = "vector"):
#         """
#         创建一个新的表并插入初始数据。
#         :param table_name: 表名。
...
ids = [1, 2, 3]
text = ["母猪产仔后，其生殖器官往往会发生了很大的变化，同时机体的抵抗力也会明显下降，此阶段若护理不当，将会影响到母猪产后机能的恢复。", "LanceDB is awesome", "Vector search is fast"]
vectors = [[0.1, 0.2, 0.3, 0.9], [0.4, 0.5, 0.6, 0.9], [0.7, 0.8, 0.9, 0.9]]

df1 = pd.DataFrame({"id": ids, "text": text, "vector": vectors})
print(df1)